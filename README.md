...
在 KOReader 上阅读微信读书中的书籍和公众号文章。支持阅读进度同步、阅读时长同步与统计，以及查看书评、划线和想法，让 Kindle、Kobo 等设备也能使用微信读书内容。
...
1. 前往[私人版 GitHub Releases](https://github.com/Yessi-cmd/weread.koplugin/releases)下载最新的 `weread.koplugin-vX.Y.Z.zip` 安装包；如果暂时没有私人版 Release，也可以检出当前源码并运行 `bash scripts/package_release.sh` 本地打包。
...
这个私人版只从 `Yessi-cmd/weread.koplugin` 的 Release 检查更新，不会安装公开上游版本。在线安装仍会校验下载地址、包大小和 SHA-256；也可以继续手动安装私人版 ZIP。插件显示名为“weread-yessi”，安装目录仍必须保持为 `weread.koplugin`。
...
│       ├── 版本 x.y.z
│       ├── 作者：finlater
│       ├── 私人版 · 更新源：Yessi-cmd/weread.koplugin
│       ├── 检查更新 / 更新到 vX.Y.Z
│       ├── 每天自动检查一次
│       └── 更新时优先使用代理
```
...
打开书籍时，如果 Wi-Fi 尚未恢复或拉取暂时失败，插件会在后台做有上限的延迟重试；手动“立即同步进度”仍会立即返回结果。阅读时长上报会等到当前阅读位置完成校验，并在“上报状态”中显示等待进度、等待网络或错误状态。

整本书下载会把章节正文和图片暂存到磁盘，最终 EPUB 在独立子进程中完成打包，并在开始前检查剩余空间。这样可以降低大书在主界面进程中的峰值内存和卡顿风险；打包阶段为保证原子替换，不能中途取消。

公众号使用微信读书的非公开 Web 接口，稳定性会受账号状态和服务端风控影响，属于实验性功能。插件会区分凭证过期、安全验证和临时限流：按提示重新扫码、先在微信读书 App/网页版完成验证，或暂停一段时间后再续期 Cookie，不要在风控期间连续重试。

缓存目录会写入 weread-yessi 归属标记。清理时只有归属与书籍 ID 一致，或可确认是当前缓存根目录下的旧版目录，才会删除；无法确认的目录会保留并提示，避免损坏用户的其他文件。

阅读 Kindle 上已有的本地漫画、PDF 或第三方电子书时，也可以只上报阅读时长：先让对应书目出现在微信读书书架中，再进入“阅读时间上报 → 选择目标书籍 → 手动设置上报书籍”选择它。之后继续在 KOReader 中打开原来的本地文件即可，不需要通过插件重新下载或阅读另一份文件。此模式不会把本地页码作为微信读书进度上传；手动目标是全局设置，切换到另一部本地作品时要同步切换目标，避免把时长记到上一部作品。
