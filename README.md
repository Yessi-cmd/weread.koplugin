...
后续可在 **微信读书 → 设置 → 更新管理** 中手动检查更新，或开启“每天自动检查一次”。为改善国内网络下的更新体验，更新请求默认优先尝试多个代理，失败后回退到 GitHub 直连；关闭“更新时优先使用代理”后则按相反顺序尝试。发现新版本时会先在可滚动页面中显示更新日志；确认安装后会显示安装包下载百分比、已下载大小，以及校验、解压和安装阶段。插件会校验发布包的 SHA-256，保留上一版备份，并提示重启 KOReader。
